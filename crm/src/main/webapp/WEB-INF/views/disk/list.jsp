                ...
                <div class="box-header with-border">
                    ...
                    <div class="box-tools pull-right">
                        <c:if test="${not empty disk}">
                            <a href="/disk/list?pid=${disk.pId}" class="btn btn-default btn-sm"><i
                                    class="fa fa-arrow-left"></i>返回上一级</a>
                        </c:if>

                        <c:choose>
                            <c:when test="${disk.type == 'file' }">
                                <a href="/disk/download?id=${disk.id}&fileName=${disk.name}"
                                   class="btn btn-info btn-sm"><i class="fa fa-download"></i> 下载</a>
                            </c:when>
                            <c:otherwise>
                                <a id="picker"><i class="btn-sm fa fa-upload">上传文件</i> </a>
                                <button id="addFolderBtn" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>
                                    新建文件夹
                                </button>
                            </c:otherwise>
                        </c:choose>
                                    ...
                                    <td colspan="4">
                                        <c:choose>
                                            <c:when test="${disk.name.endsWith('.pdf') or disk.name.endsWith('.jpg') or disk.name.endsWith('.gif') or disk.name.endsWith('.jpeg') or disk.name.endsWith('.png')}">
                                                <a href="/disk/download?id=${disk.id}" target="_blank"
                                                   class="btn btn-default btn-sm"><i class="fa fa-search"></i> 预览</a>
                                                <a href="/disk/download?id=${disk.id}&fileName=${disk.name}"
                                                   class="btn btn-info btn-sm"><i class="fa fa-download"></i> 下载</a>
                                            </c:when>
                                            <c:otherwise>
                            ...
                            <c:otherwise>
                                ...
                                <c:forEach items="${diskList}" var="disk">
                                    <tr class="tr" rel="${disk.id }">
                                        ...
                                        <td class='td' rel="${disk.id }"><fmt:formatDate value="${disk.updateTime}"
                                            ...
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <c:choose>
                                                        ...
                                                        <c:otherwise>
                                                            <li>
                                                                <a href="/disk/download?id=${disk.id}&fileName=${disk.name}">下载</a>
                                                            </li>
                                                        </c:otherwise>
                                                    </c:choose>
